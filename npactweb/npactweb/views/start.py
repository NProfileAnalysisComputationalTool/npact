...
from npactweb import is_clean_path, getrelpath, library_root
...
class StartForm(forms.Form):
    ...
    url = forms.URLField(label="From URL",
                         widget=get_ti(50),
                         required=False)
    pastein = forms.CharField(
        label="Paste in as text",
        widget=forms.Textarea(attrs={'rows': 3, 'cols': ""}),
        required=False)
    entrez_search_term = forms.CharField(
        label="Accession Number", required=False)
...
def re_search(req):
    if req.REQUEST.get('entrez_search_term'):
        return render_to_response(
            'start.html', {'form': StartForm(req.REQUEST)},
            context_instance=RequestContext(req))
    else:
        return view(req)
