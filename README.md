...
npactweb component). The pynpact component can be used directly on the
...
in a
[format](http://www.pip-installer.org/en/latest/requirements.html)
...
* Django (1.3): Used to build the website interface. Upgrading this
...
Primarily this should be run underneath a fcgi webserver. Look at
`apache.conf` for an example of how to do that.

### Development mode

Django does have a builtin webserver that will work for local
installations, to use it.

1. edit `npactweb/settings.py` to change DEBUG from False to True at
   the top of the file.
2. `source ve/bin/activate[.csh]`
3. `python manage.py runserver`
4. open [http://127.0.0.1:8000/npact/]()
...
[Django](http://djangoproject.com/). See `README.md` in that folder.
...
`npactweb/npactweb/templates`. This uses the
[Django template language](https://docs.djangoproject.com/en/1.3/topics/templates/). To
