  ...
  while(fgets(longstr,198,input) && !feof(input) && (start+n)<=end)
  {
    for(i=0;i<strlen(longstr)-1 && (start+n)<=end;++i)
    {
      ...
      ++m;
        if(m>=start && m<=end)
        {
          if(longstr[i]=='c' || longstr[i]=='g') { if(box[n%window]!='S') { ++S[(m-1)%(period)]; box[n%window]='S'; } ++n; }
          else if(longstr[i]=='a' || longstr[i]=='t' || longstr[i]=='u') { if(box[n%window]=='S') { --S[(m-1)%(period)]; box[n%window]='W'; } ++n; }
